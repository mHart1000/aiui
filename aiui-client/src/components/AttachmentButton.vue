    ...
    <q-btn icon="add" color="secondary" round flat>
      ...
      <q-menu auto-close anchor="top left" self="bottom left">
        <q-list style="min-width: 190px">
          <q-item clickable :disable="!visionSupported" @click="pickFiles">
            <q-item-section avatar>
              <q-icon name="image" />
            </q-item-section>
            <q-item-section>Attach image</q-item-section>
          </q-item>
          <q-item v-if="!visionSupported" dense class="attach-hint">
          ...
          <q-item v-if="showNewChat" clickable @click="$emit('new-chat')">
            <q-item-section avatar>
...
export default {
  ...
  methods: {
    pickFiles () {
      if (!this.visionSupported) return
